...
pub fn split_into_chunks(text: &str, file_name: &str) -> Vec<TextChunk> {
    ...
    for paragraph in paragraphs {
        ...
        if paragraph_token_count > TARGET_TOKENS {
            ...
            for sentence in sentences {
                ...
                if buffer_token_count + sentence_token_count > TARGET_TOKENS && !sentence_buffer.is_empty() {
                    ...
                    let overlap_start = sentence_buffer
                        .char_indices()
                        .nth(sentence_buffer.chars().count().saturating_sub(OVERLAP_TOKENS * 4)) // Approximate char count for overlap tokens
                        .map(|(i, _)| i)
                        .unwrap_or(0);
            ...
            if current_token_count + paragraph_token_count > TARGET_TOKENS && !current_chunk.is_empty() {
                ...
                let overlap_start = current_chunk
                    .char_indices()
                    .nth(current_chunk.chars().count().saturating_sub(OVERLAP_TOKENS * 4)) // Approximate char count for overlap tokens
                    .map(|(i, _)| i)
                    .unwrap_or(0);
    ...
    for chunk in chunks {
        if chunk.token_count > TARGET_TOKENS * 3 {
            ...
            let TextChunk { text, token_count: _, file_name } = chunk;
